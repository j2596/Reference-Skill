...
4. 使用 Chrome 打开知网，筛选候选文献，复制 GB/T 7714-2025 引文条目，去除 DOI 部分，关闭引用弹窗后只从详情页下载 PDF。
5. 阅读本地 PDF 全文，抽取能支撑正文论点的完整证据句。
6. 筛选掉无 PDF、下载失败、不可读、相关性弱或重复的文献。
7. 修改或补写正文，使引用句真实契合 PDF 证据，再按正文首次出现顺序插入引用编号，避免堆叠式引用。
...
6. 除非原文已经直接契合 PDF 证据，否则必须修改或补写正文，不能只追加引用编号。
7. 引用对应核验说明应使用中文字段，并标明第几章第几节或稳定段落位置。
8. 写入 Word、整理表和核验说明的 GB/T 7714-2025 条目应去除 DOI 及其后续内容。
